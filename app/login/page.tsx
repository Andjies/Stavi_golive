  ...
  return (
    <div className="paper-texture min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 shadow-xl border border-stavi-paper">
        ...
        <h1 className="font-display text-3xl font-semibold mb-2">{t("auth.welcomeBack")}</h1>
        <p className="text-stavi-inkSoft mb-8">{t("auth.login")}</p>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-semibold mb-1.5">{t("auth.password")}</label><input type="password" required value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-stavi-cream border border-stavi-paper focus:outline-none focus:border-stavi-terracotta" /></div>
          <button disabled={loading} className="btn-primary w-full disabled:opacity-50">{loading ? <Loader2 className="animate-spin" size={18} /> : t("auth.login")}</button>
        </form>
        <p className="mt-6 text-sm text-stavi-inkSoft text-center">{t("auth.noAccount")} <Link href="/register" className="text-stavi-terracotta font-semibold">{t("nav.register")}</Link></p>
      </div>
    </div>
  )
}