...
export async function POST(req: NextRequest, { params }: { params: Promise<{ cid: string }> }) {
  ...
  const custom = await prisma.customization.findUnique({ where: { id: cid } })
  ...
  await prisma.customization.update({ where: { id: cid }, data: { pdfData: buf, pdfFilename: file.name, status: "delivered", deliveredAt: new Date() } })
