...
export async function GET(req: NextRequest, { params }: { params: Promise<{ cid: string }> }) {
  ...
  const doc = await prisma.customization.findUnique({ where: { id: cid } })
  ...
  return new NextResponse(doc.pdfData as unknown as ArrayBuffer, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${doc.pdfFilename||"stavi-custom.pdf"}"` } })
}
